"""Interactive select menus used by the setup_settings cog (cogs/setup.py)."""
...
class setup_view(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        await self.bot.get_cog("setup_settings").headcategorysetup_menu(interaction,hcategory)
...
class selectheadcategory(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        subkeyss = await db.subkeys.find({"database":headkeys}).to_list(length=None)
        if len(subkeyss) != 0:
            selectOption = []
            count =0
            ...
            goods = await db.goodsdb.find( { "type": { "$not": { "$regex": "^keys.*" } } } ).to_list(length=None)
            selectOption = []
...
class SelectGoodsType(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if value == "manual":
            ...
            if value == "keys":
                headkeys = await db.goodsdb.find({"guilds":interaction.guild_id,"type":"keys"}).to_list(length=None)
                if len(headkeys) != 0:
                    selectOption = []
                    count =0
                    description = ""
                ...
                good = await db.goodsdb.find_one({"guilds":interaction.guild_id,"_id":ObjectId(value)})
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if int(value) == 1:
            ...
            if event2:
                if   "run" in event2:
                    view = "idk"
                    await self.bot.get_cog("setup_settings").productsmanager(interaction,hcategory,scategory)
                else:
        ...
        if value == "00":
            ...
            await self.bot.get_cog("setup_settings").subcategorys_setup(interaction, hcategory)
        else :
            if view is None:
                await self.bot.get_cog("setup_settings").subcategory_menu(interaction, hcategory,scategory)
                return
...
class selectevent(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if int(value) == 1:
            ...
            embtry = await db.embedstylesdb.find_one({"guildid":interaction.guild_id,"type":"submenu","headcategory":f"category{hcategory}"})
            if embtry is None:
            ...
            changeto = "products"
            embtry = await db.embedstylesdb.find_one({"guildid":interaction.guild_id,"type":"productsmenu","headcategory":f"category{hcategory}","subcategory":f"subcategory{scategory}"})
            if embtry is None:
            ...
            embtry2 = await db.embedstylesdb.find_one({"guildid":interaction.guild_id,"type":"payments","headcategory":f"category{hcategory}","subcategory":f"subcategory{scategory}"})
            if not embtry2:
            ...
            changeto = "ticket"
        if not scategory:
            await db.headcategorysdb.update_one({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}"}, {"$set":{"run":changeto}})
        else:
            await db.subcategoriesdb.update_one({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}", "subcategory":f"subcategory{scategory}"}, {"$set":{"run":changeto}})
...
class setupstylehead(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if int(value) == 1:
            if await db.embedstylesdb.find_one({"guildid":interaction.guild_id, "type":"headmenu"}) is None:
            ...
            if   styledb is not None:
                if int(value) == 2:
                    ...
                    if msg:
                        ...
                        await db.embedstylesdb.update_one({"_id":styledb["_id"]}, {"$set":{"description":msg.content}})
                    ...
                    msg = await helpers.waitforrespon(self,interaction.channel,interaction.user,"msg",180)
                    if msg:
                        await db.embedstylesdb.update_one({"_id":styledb["_id"]}, {"$set":{"footer-text":msg.content}})
                    ...
                    if isinstance(image, str):
                            ...
                            await db.embedstylesdb.update_one({"_id":styledb["_id"]}, {"$set":{"image":image}})
                    ...
                    image ,msg = await helpers.waitforrespon(self,interaction.channel,interaction.user,"image_n",180)
                    if isinstance(image, str):
                            await db.embedstylesdb.update_one({"_id":styledb["_id"]}, {"$set":{"icon":image}})
                    ...
                    if msg:
                        ...
                        await db.embedstylesdb.update_one({"_id":styledb["_id"]}, {"$set":{"title":msg.content}})
                    ...
                    image ,msg = await helpers.waitforrespon(self,interaction.channel,interaction.user,"image_n",180)
                    if isinstance(image, str):
                        await db.embedstylesdb.update_one({"_id":styledb["_id"]}, {"$set":{"footer-icon":image}})
                ...
                if value == "00":
                    ...
                    await self.bot.get_cog("setup_settings").presetup2_menu(interaction)
            
                else :
                    
                    
                    await self.bot.get_cog("setup_settings").setupstylehead_menu(interaction)
...
class presetup(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if int(value) == 1:
            ...
            await self.bot.get_cog("setup_settings").setup_menu(interaction )
                    
            
        elif int(value) == 3:
            await interaction.response.defer()
            await self.bot.get_cog("setup_settings").presetup_menu(interaction)
        elif int(value) == 4:
            goods = await db.goodsdb.find( { "type": { "$not": { "$regex": "^keys.*" } } } ).to_list(length=None)
            selectOption = []
            emojilist = ["🔴","🟠","🟡","🟢","🔵","🟣","🟤","⚫","⚪"]
...
class goodsmanager(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        value = self.values[0]
        if value == "add":
            gmdb =  await db.goodsdb.find({"guilds":interaction.guild_id}).to_list(length=None)
            ...
            db.keysdbs = await db.goodsdb.find({"guilds":interaction.guild_id,"type":"keys"}).to_list(length=None)
        ...
        if value == "add":
            ...
            if msg:
                ...
                if len(name) <= 50: 
                    ...
                    trys = await db.goodsdb.find({"guilds":interaction.guild_id,"label":name, "type":"keys"}).to_list(length=None)
                    if len(trys) == 0 :
                        _id = db.goodsdb.insert_one({"guilds":[interaction.guild.id],"label":name,"type":"keys"}).inserted_id
                        await self.bot.get_cog("setup_settings").keysdb(interaction,_id)
        ...
        if value == "add": 
            ...
            if msg:
                ...
                if len(name) <= 50: 
                    ...
                    trys = await db.subkeys.find({"guildid":interaction.guild_id,"label":name, "type":"keys"}).to_list(length=None)
                    if len(trys) == 0 :
                        _id = db.subkeys.insert_one({"guildid":interaction.guild.id,"label":name,"database":ObjectId(databaseid)}).inserted_id
                        await self.bot.get_cog("setup_settings").addkeys(interaction.channel,interaction.user,databaseid,_id)
            ...
            selectOption = []
            database = await db.subkeys.find_one({"_id":ObjectId(value)})
...
class keysmanager(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if value == "add":
            ...
            await self.bot.get_cog("setup_settings").addkeys(interaction.channel,interaction.user,databaseid,interaction)
        
        elif value == "2":
            selectOption = []
            description = "Select key to edit"
            number = 0
            keylib = await db.keysdb.find({"database":ObjectId(databaseid)}).to_list(length=None)
            for baldb in keylib:
...
class keyssettings(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        keyid = self.keyid
        value = self.values[0]
        keydb = await db.keysdb.find_one({"_id":ObjectId(keyid)}) # it is in obid but whatever
        ...
        if value == "show":
            ...
            if msg:
                await db.keysdb.update_one({'userid': interaction.user.id}, {'$set': {'key': msg.content}})
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        for slotid in self.values[0]:
            if slotid == "0":
                ...
                ids += f"{slotid}-"
        await self.bot.get_cog("setup_settings").commendbotsetup(interaction,ids)
...
class goodsadd(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        type = self.values[0]
        subdbs = await db.subdb.find_one({"ownerid":interaction.user.id,"disabled":False})
        if  subdbs:
                
            if type == "1":
                balsdb = await db.balancesdb.find({"userid":interaction.user.id}).to_list(length=None)
                if len(balsdb) == 1:
                    slots = "0"
                    ...
                    for baldb in balsdb:
                        ...
                        slot = await db.slotsdb.find_one({"_id":baldb["slot_id"]})
                        if slot["enable"]:
...
class setupcategory(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if int(value) == 1:
            ...
            events = await db.headcategorysdb.find_one({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}"})
            if events is not None:
                ...
                if "subcategory" not in spcategory["run"]: 
                    back =False
                    await self.bot.get_cog("setup_settings").productsmanager(interaction,hcategory)
                else:
            ...
            menu = await db.headcategorysdb.find_one({"headcategory":f"category{hcategory}", "guildid":interaction.guild_id,})
            if menu is not None:
                ...
                await db.headcategorysdb.update_one({"guildid":interaction.guild_id,"headcategory":f"category{hcategory}"}, {"$set":{"enabled":(not menu["enabled"])}})
            ...
            if view.value is None:
                ...
                print("Confirmed...")
                await db.productsdb.delete_many({"guildid":interaction.guild_id, "headcategory": f"category{hcategory}"})
                
                await db.subcategoriesdb.delete_many({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}" })
                await db.headcategorysdb.delete_many({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}"})
                await db.embedstylesdb.delete_many({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}"})
        ...
        if int(value) == 1:
            ...
            try:
                ...
                if category is None:
                    ...
                    await db.headcategorysdb.update_one({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}"}, {"$set":{"categoryid":category.id}})
            ...
            event = await db.headcategorysdb.find_one({"guildid":interaction.guild_id, "headcategory":f"category{hcategory}"}) 
            if event  and "categoryid" in event:
                ...
                try:
                    ...
                    if category is None:
                        ...
                        await db.headcategorysdb.update_one({"_id": event["_id"]}, {"$set":{"categoryid":category.id}})
...
class paymentseditit(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if int(value) == 2:
            ...
            color , msg = await helpers.waitforrespon(self,interaction.channel,interaction.user,"hex",180)
            if color:
                    await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"headmenu"}, {"$set":{"color":color}})
            ...
            try:
                ...
                await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"headmenu"}, {"$set":{"description":text.content}})
            ...
            try:
                ...
                await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"headmenu"}, {"$set":{"footer-text":text.content}})
            ...
            try:
                ...
                if 'https://' in text.content  or 'http://' in text.content:
                    await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"headmenu"}, {"$set":{"image":text.content}})
                    ...
                    await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"headmenu"}, {"$unset":{"image":""}})
                    await interaction.send(content="Successfully removed image.")
            ...
            try:
                ...
                if 'https://' in text.content  or 'http://' in text.content:
                    ...
                    await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"headmenu"}, {"$unset":{"icon":""}})
            ...
            try:
                ...
                await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"payments"}, {"$set":{"label":text.content}})
            ...
            try:
                ...
                if 'https://' in text.content  or 'http://' in text.content:
                    await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"payments"}, {"$set":{"footer-icon":text.content}})
                    await interaction.send(content=f"Successfully updated footer icon to {text.content}",delete_after=30)
                        
                elif text.content.startswith("0") or text.content.startswith("no"):
                    await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"payments"}, {"$unset":{"footer-icon":""}})
            ...
            try:
                ...
                await text.delete()
                await db.embedstylesdb.update_one({"guildid":interaction.guild_id, "type":"payments"}, {"$set":{"text":text.content}})
